...
main_menu.add_cascade(label='IPaD Theme', menu=color_theme)
...
# font box
font_tuples = tk.font.families()  # all font are there
...
# functionality of change txt and size
...
def copy_func(event=None):
    text_editor.event_generate("<<Copy>>")


edit.add_command(label='Copy', accelerator='Ctrl+C',command=copy_func)


def paste_func(event=None):
    text_editor.event_generate("<<Paste>>")


edit.add_command(label='Paste', accelerator='Ctrl+V', command=paste_func)
...
edit.add_command(label='Cut', accelerator='Ctrl+X', command=cut_func)
...
# view menu commands Check button
## view check button
